...
def find_logo_path():
    possible_paths = [
        os.path.join(settings.BASE_DIR, 'mainapp', 'static', 'logo.png'),
        os.path.join(settings.BASE_DIR, 'static', 'logo.png'),
        os.path.join(settings.BASE_DIR, 'mainapp', 'static', 'mainapp', 'logo.png'),
    ]
    
    if hasattr(settings, 'STATIC_ROOT') and settings.STATIC_ROOT:
        possible_paths.append(os.path.join(settings.STATIC_ROOT, 'mainapp', 'static', 'logo.png'))
        possible_paths.append(os.path.join(settings.STATIC_ROOT, 'logo.png'))
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            total_marks = sum(form.cleaned_data[field] for field in form.cleaned_data 
                            if field not in ['guide_name', 'presentation_date', 'start_time', 
                                          'end_time', 'project_title', 'mentor_name'])
            ...
            if role == 'mentor':
                ...
                examiner_assignment.submission_status = 'Submitted'
        ...
        if role == 'mentor':
            ...
            # Add mentor name to initial data
            initial_data['mentor_name'] = student.professor.get_full_name() or student.professor.username
        else:
